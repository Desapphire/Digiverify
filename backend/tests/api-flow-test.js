...
async function main() {
    ...
    if (propertyRes.data.blockchainTxHash) {
        console.log(`   ⛓️  Blockchain Tx: ${propertyRes.data.blockchainTxHash}`);
    }
    ...
    if (approvePropRes.data.blockchainTxHash) {
        console.log(`   ⛓️  Blockchain Tx: ${approvePropRes.data.blockchainTxHash}`);
    }
    ...
    if (saleRes.data.blockchainTxHash) {
        console.log(`   ⛓️  Blockchain Tx: ${saleRes.data.blockchainTxHash}`);
    }
    // 🔟 BUYER SIGN SALE
    console.log(`\n🖊️  Buyer signing sale...`);
    ...
    if (buyerSignRes.data.blockchainTxHash) {
        console.log(`   ⛓️  Blockchain Tx: ${buyerSignRes.data.blockchainTxHash}`);
    }
    ...
    if (confirmRes.data.blockchainTxHash) {
        console.log(`   ⛓️  Blockchain Tx: ${confirmRes.data.blockchainTxHash}`);
    }

    // 1️⃣3️⃣ AUTHORITY APPROVE SALE (ADMIN)
    console.log(`\n🏛️  Authority approving sale (Admin)...`);
    ...
    if (approveSaleRes.data.blockchainTxHash) {
        console.log(`   ⛓️  Blockchain Tx: ${approveSaleRes.data.blockchainTxHash}`);
    }
    ...
    if (completeRes.data.blockchainTxHash) {
        console.log(`   ⛓️  Blockchain Tx: ${completeRes.data.blockchainTxHash}`);
    }
